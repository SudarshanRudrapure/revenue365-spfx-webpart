...
const Bundles: React.FC = () => {
  ...
  const handleEdit = (row: Record<string, any>) => {
    setBundleName(row.Title || '');
    setBundleDesc(row.Description || '');
    setBundleHSN(row.HSN_SAC || '');
  ...
  const handleToggle = (itemName: string) => {
    const current = selectedItems
      ? selectedItems.split(',').map(i => i.trim())
      : [];
  ...
  return (
    <>
      ...
      <DataTable
        ...
        isLoading={isLoading}
        onAdd={() => {
          setGeneratedID(generateID('BUN', items, 'BundlesID'));
          clearForm();
          setIsPanelOpen(true);
        }}
        onEdit={handleEdit}
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h2 style={{ margin: 0, fontSize: '16px' }}>
                {isEditing ? '✏️ Edit Bundle' : 'Add Bundle'}
              </h2>
              <button
                onClick={() => { setIsPanelOpen(false); clearForm(); }}
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', fontWeight: 600, fontSize: '13px', marginBottom: 5 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input
                  style={{ ...inputStyle, background: '#f9fafb', color: '#4b5563' }}
                  value={isEditing ? (items.find(i => i.Id === editItemId)?.BundlesID || '') : generatedID}
                  readOnly
                />
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input
                  ...
                  value={bundleHSN}
                  onChange={e => setBundleHSN(e.target.value)}
                />
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', fontWeight: 600, fontSize: '13px', marginBottom: 5 }}>
                  Bundle Items <span style={{ color: '#b91c1c' }}>*</span>
                </label>

                <div className={styles.bundleSelectWrapper}>
                  <div
                    className={styles.bundleSelectTrigger}
                  ...
                  {isDropdownOpen && (
                    <div className={styles.bundleSelectDropdown}>
                      ...
                      {products.length === 0 && services.length === 0 && subscriptions.length === 0 && (
                        <div className={styles.bundleSelectEmpty}>
                          No items found. Add Products, Services or Subscriptions first!
                        </div>
                      )}
            ...
            <div style={{ padding: '16px 20px', borderTop: '1px solid #e5e7eb', display: 'flex', gap: 12 }}>
              <button
                ...
                style={{
                  ...
                  borderRadius: 4, padding: '9px 24px', fontSize: 14,
              ...
              <button
                style={{
                  ...
                  padding: '9px 24px', fontSize: 14, cursor: 'pointer',
                }}
                onClick={() => { setIsPanelOpen(false); clearForm(); }}
              >
                Cancel
